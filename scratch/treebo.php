<?php
$url = "https://commons.wikimedia.org/w/api.php?action=query&list=search&srsearch=" . urlencode("Treebo") . "&srnamespace=6&format=json&srlimit=5";
        ...
        if(isset($res2['query']['pages'])) {
            foreach($res2['query']['pages'] as $page) {
                if(isset($page['imageinfo'][0]['thumburl'])) {
                    echo "Treebo -> " . $page['imageinfo'][0]['thumburl'] . "\n";
                }
            }
        }
    }
